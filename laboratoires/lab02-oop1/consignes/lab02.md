...
Le constructeur **doit** toujours être le même nom que la classe. Il s'agit d'une fonction spéciale qui est appeler lors de l'instantiation d'un objet de notre classe. Dans notre cas, nous avons deux constructeurs : un constructeur par défaut (sans paramètre) et un constructeur qui nous permet d'initialiser tous les attributs de notre instance. Toutefois dans notre en-tête `.h` nous n'avons que définit les constructeurs, nous avons besoins de les implémenter (c'est-à-dire, écrire le code) pour initialiser notre objet. Il faut donc créer une fichier `tache.cpp` avec le code suivant :
...
Nous allons donc parler du concept derrière `this` et de son importance dans la programmation orienté objet. Comme nous l'avons déjà dit, la classe est une description : si je n'instancie pas d'objet de cette classe dans mon code, ce code ne sera jamais exécuté. Mais si j'instancie deux instances, comment faire la distinction entre les deux ? Voici un exemple :
...
Vous remarquerez que le constructeur est placé dans une autre section que celle contenant les attributs. Sachez également que tous les éléments présents dans une classe, que se soit des attributs, des constructeurs, ou autres éléments à venir, sont des **membres** de la classe.

### Les méthodes

Donc maintenant que nous avons une classe, qui contient des attributs (variables) lié entre elle - une sorte de contenant qui définit une **entité**, qu'est-ce que la programmation orienté objet nous offres pour nous aider ? L'une d'entre elle est le concept de méthode.

Pour comprendre ce qu'est une méthode, il faut ce poser la question suivante : qu'est-ce que mon objet de ma classe va exister, quelles sont les actions que je veux poser sur elle et ses attributs ? Par exemple, un objet de classe `Tache` pourrait avoir les actions suivantes :
...
Comme ce sont des actions que l'on peut faire sur une tâche, on va vouloir créer des fonctions dans la classe : on les appels les méthodes. On modifie notre `tache.h` tel que :
...
Le mot clé `private` va cacher les variables et seul les méthodes de la classe pourront les modifier. Notez que dans le livre, les attributs privées ont un préfixe `m_` alors qu'ici c'est uniquement `_`. Sachez qu'il s'agit de convention et qu'elle peut différer d'un projet à l'autre. Rien ne nous empêche de mettre aucun préfix. On vous demande cependant, dans le cadre du cours, de suivre la nomenclature décrite dans le standard de programmation.
...
class Personne {
    ...
    + Tache *obtenirTache(int index)
    + void changerPrenom(std::string prenom)
...
Donc on voit notre classe `Tache` liée à notre classe `Personne`. Le losange noir signifie que la relation entre les deux est une de composition : on dit que la `Personne` est composée (au moins) d'une `Tache`. Pour ceux qui on l'oeil du Lynx, vous remarquerez qu'un drôle de "constructeur" avec une `~` a été mis. On va regarder tout ça de près.
...
On remarque que le destructeur porte le même nom que la classe avec un tilde (`~`). Le destructeur n'aura jamais de paramètres et devrait généralement s'occuper à des tâches de libérations de ressource, tel désalouer un pointeur, fermer un fichier, etc.
